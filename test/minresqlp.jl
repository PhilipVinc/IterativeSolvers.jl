...
@testset "Hermitian Matrix{$T}" for T in (Float32, Float64, ComplexF32, ComplexF64)
    ...
    x1, hist1 = minresqlp(A, b, maxiter = 10n, tol = tol, log = true)
    x2, hist2 = minresqlp!(x0, A, b, maxiter = 10n, tol = tol, log = true)
...
@testset "Skew-Hermitian Matrix{$T}" for T in (Float32, Float64, ComplexF32, ComplexF64)
    ...
    tol = sqrt(eps(real(T)))
    x_approx, hist = minresqlp(A, b, skew_hermitian = true, maxiter = 10n, tol = tol, log = true)
...
@testset "SparseMatrixCSC{$T, $Ti}" for T in (Float32, Float64, ComplexF32, ComplexF64), Ti in (Int64, Int32)
    ...
    x_approx, hist = minresqlp(A, b, maxiter = 10n, tol = tol, log = true)
